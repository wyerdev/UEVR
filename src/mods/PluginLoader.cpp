...
void PluginLoader::early_init() try {
    ...
    // [fork] variant-isolation: only ever scan this build's own variant
    // subdirectory. The unqualified root and other variants' directories are
    // never touched, so several builds can coexist under %APPDATA%.
    spdlog::info("[PluginLoader] Build variant: '{}'", UEVR_VARIANT_ID);

    const auto plugin_path = Framework::get_persistent_dir() / "plugins" / UEVR_VARIANT_ID;
    const auto global_plugins_path = Framework::get_persistent_dir() / ".." / "UEVR" / "plugins" / UEVR_VARIANT_ID;
    ...
    auto load_plugins_from_dir = [this](std::filesystem::path path) {
        spdlog::info("[PluginLoader] Scanning {}", path.string());

        if (!fs::exists(path) || !fs::is_directory(path)) {
            spdlog::info("[PluginLoader] Skipping {} (not a directory)", path.string());
            return;
        }
        ...
        for (auto&& entry : fs::directory_iterator{path}) {
            ...
            if (path.has_extension() && path.extension() == ".dll") {
                ...
                // [fork] variant-isolation: identity handshake. A DLL sitting in
                // our variant directory still has to say it was built for this
                // variant, otherwise it is a stale copy or a manual mistake.
                using variant_id_fn = const char* (*)();
                const auto variant_id_export =
                    (variant_id_fn)GetProcAddress(module, UEVR_PLUGIN_VARIANT_EXPORT);
                ...
                if (variant_id_export == nullptr) {
                    ...
                    m_plugin_load_errors.emplace(path.stem().string(), "Not built for variant '" UEVR_VARIANT_ID "'");
                    FreeLibrary(module);
                    continue;
                }
                ...
                if (plugin_variant == nullptr || std::string_view{plugin_variant} != UEVR_VARIANT_ID) {
                    ...
                    m_plugin_load_errors.emplace(path.stem().string(), "Variant mismatch");
                    FreeLibrary(module);
                    continue;
                }

                spdlog::info("[PluginLoader] Loaded {} (variant '{}')", path.string(), plugin_variant);
...
std::optional<std::string> PluginLoader::on_initialize_d3d_thread() {
    ...
    // Dumper mode: no D3D hook installed, so renderer_data stays null. Plugins
    // that only use UObject reflection (the intended use case for dumper mode)
    // never touch device / swapchain / command_queue, so this is fine.
    // See src/DumperMode.hpp. Without this escape hatch, the D3D-required
    // branch below returns early and plugins never get uevr_plugin_initialize
    // called — their HTTP servers / UI / whatever never start.
    if (uevr::is_dumper_mode()) {
        spdlog::info("[PluginLoader] Dumper mode: skipping renderer data setup, plugin init proceeds with null renderer.");
    } else if (uevr::g_renderer_data.renderer_type == UEVR_RENDERER_D3D11) {
    ...
    // Plugins do NOT read settings files themselves anymore.
    uevr::settings_registry::apply_auto_preset();
...
void PluginLoader::attempt_unload_plugins() {
    {
        ...
        uevr::settings_registry::flush_now();
        uevr::settings_registry::clear();
