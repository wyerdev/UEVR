...
3. **Run `install-plugins.bat`** from the plugins zip — installs all plugin DLLs and built-in presets globally
4. **Inject as usual** and open the UEVR menu (**Insert** or **L3+R3**) to configure plugins or load a preset

> **Note:** These plugins only work with this patched fork. They will **not** load on stock UEVR nightly — the version guard safely skips them.

### Manual plugin install (optional)

Instead of `install-plugins.bat`, you can copy files manually:

**Global (all games):** Copy `*Plugin.dll` and `*-LICENSE.txt` to `%APPDATA%\UnrealVRMod\UEVR\plugins\`
...
| What | Where |
|------|-------|
| Global plugins | `%APPDATA%\UnrealVRMod\UEVR\plugins\` — loaded for **all** games |
| Per-game plugins | `%APPDATA%\UnrealVRMod\<game_executable>\plugins\` — loaded for that game only |
...
Run `uninstall-plugins.bat` from the release folder or from `%APPDATA%\UnrealVRMod\UEVR\plugins\`. Removes all plugin DLLs, licenses, and built-in presets. Your saved presets and per-game settings are kept.
