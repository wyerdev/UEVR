...
<!-- [fork] afw: this branch builds out-of-source into `A:\UEVR-build\afw`.
     Prefer `cmd.exe //c "build.bat"` from the repo root over invoking cmake
     directly — `build.bat` owns the build directory, regenerates the commit
     hash, and runs the stale-object guard. -->
<!-- [fork] build: correct the CMake configure command for this fork's build layout -->
```
cmake -S . -B "A:/UEVR-build/afw" -G "Visual Studio 17 2022" -A x64 -DCMAKE_BUILD_TYPE=Release
cmake --build "A:/UEVR-build/afw" --config Release --target uevr
```
...
```
cmake --build "A:/UEVR-build/afw" --config Release --target fakehdr_plugin
```

Or build all targets at once. Plugin DLLs are output to `A:\UEVR-build\afw\Release\` with numeric prefixes (e.g. `07_FakeHDRShader.dll`).
...
ls -la <changed-header>.hpp \
       "A:/UEVR-build/afw/<consumer-target>.dir/Release"/*.obj
```
...
rm -f "A:/UEVR-build/afw/plugin_renderlib.dir/Release"/effect_runtime*.obj
cmd.exe //c "build.bat"
```
