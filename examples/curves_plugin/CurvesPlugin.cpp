...
public:
    ...
    void on_pre_render_vr_framework_dx12() override {
        ...
        // Step 2: Draw Curves from copy_tex -> result_tex
...
private:
    ...
    bool ensure_dx12_copy_texture(ID3D12Device* device, ID3D12Resource* src, DX12TargetState& ts) {
        ...
        if (FAILED(device->CreateCommittedResource(&hp, D3D12_HEAP_FLAG_NONE, &result_desc,
            ...
            API::get()->log_error("[Curves DX12] Failed to create result texture");
            ts.reset();
            return false;
        }
        ...
        API::get()->log_info("[Curves DX12] Target textures ready: %ux%u fmt %u", ts.width, ts.height, (unsigned)resolved_fmt);
        return true;
    }
};
