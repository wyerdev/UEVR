...
public:
    ...
    std::filesystem::path get_settings_path() { return API::get()->get_persistent_dir(L"vibrance_settings.txt"); }
    void save_settings() { try { std::ofstream f(get_settings_path()); if (f.is_open()) f << m_enabled << "\n" << m_vibrance << "\n" << m_balance[0] << " " << m_balance[1] << " " << m_balance[2] << "\n"; } catch (...) {} }
    void load_settings() {
