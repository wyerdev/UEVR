...
static const char* g_tonemap_vs_src = R"(
...
static constexpr const char* TM_VERSION = "1.0.0";

class TonemapPlugin : public uevr::Plugin {
public:
    float m_gamma = 1.0f, m_exposure = 0.0f, m_saturation = 0.0f, m_bleach = 0.0f, m_defog = 0.0f;
    float m_fog_color[3] = {0.0f, 0.0f, 1.0f};
    ...
    TonemapParamsCB* m_dx12_cb_mapped = nullptr; DXGI_FORMAT m_dx12_rt_format = DXGI_FORMAT_UNKNOWN; bool m_dx12_ready = false;
    ...
    void load_settings() {
        try { std::ifstream f(get_settings_path()); if (f.is_open()) { int e; if (f >> e) m_enabled = (e != 0);
            f >> m_gamma >> m_exposure >> m_saturation >> m_bleach >> m_defog >> m_fog_color[0] >> m_fog_color[1] >> m_fog_color[2];
            m_gamma = max(0.0f, min(2.0f, m_gamma)); m_exposure = max(-1.0f, min(1.0f, m_exposure)); m_saturation = max(-1.0f, min(1.0f, m_saturation));
            m_bleach = max(0.0f, min(1.0f, m_bleach)); m_defog = max(0.0f, min(1.0f, m_defog)); } } catch (...) {}
    }

    void fill_cb(TonemapParamsCB* p) { p->Gamma = m_gamma; p->Exposure = m_exposure; p->Saturation = m_saturation; p->Bleach = m_bleach; p->Defog = m_defog; memcpy(p->FogColor, m_fog_color, sizeof(float)*3); }
...
private:
    ...
    bool init_shaders_dx11(ID3D11Device* device) {
        ...
        if (FAILED(D3DCompile(g_tonemap_vs_src, strlen(g_tonemap_vs_src), "VS", nullptr, nullptr, "main", "vs_5_0", 0, 0, &vsb, &vse))) return false;
        if (FAILED(D3DCompile(g_tonemap_ps_src, strlen(g_tonemap_ps_src), "PS", nullptr, nullptr, "main", "ps_5_0", 0, 0, &psb, &pse))) { if (pse) API::get()->log_error("[Tonemap] PS: %s", (const char*)pse->GetBufferPointer()); return false; }
        ...
        D3D11_BUFFER_DESC cbd{}; cbd.ByteWidth = sizeof(TonemapParamsCB); cbd.Usage = D3D11_USAGE_DYNAMIC; cbd.BindFlags = D3D11_BIND_CONSTANT_BUFFER; cbd.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
    ...
    void apply_effect_dx11(ID3D11Texture2D* target) {
        ...
        D3D11_MAPPED_SUBRESOURCE mapped{}; if (SUCCEEDED(ctx->Map(m_cb.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped))) { fill_cb((TonemapParamsCB*)mapped.pData); ctx->Unmap(m_cb.Get(), 0); }
    ...
    bool init_dx12(ID3D12Device* device, DXGI_FORMAT rt_format) {
        ...
        ComPtr<ID3DBlob> vsb, vse, psb, pse;
        if (FAILED(D3DCompile(g_tonemap_vs_src, strlen(g_tonemap_vs_src), "VS", nullptr, nullptr, "main", "vs_5_0", 0, 0, &vsb, &vse))) return false;
        if (FAILED(D3DCompile(g_tonemap_ps_src, strlen(g_tonemap_ps_src), "PS", nullptr, nullptr, "main", "ps_5_0", 0, 0, &psb, &pse))) return false;
